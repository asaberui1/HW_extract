...
def calculate_heatwave_threshold(input_path, output_path="./tm_threshold.nc", start_year=1981, end_year=2010):
    """
    计算高温热浪（Heatwave）的阈值。

    参数：
        input_path: str, 输入温度文件路径模板（例如"/data6/huangty/ERA5/ERA5_downloads/era5_<year>_<month>.nc"）。
        output_path: str, 输出阈值文件路径（例如"./tm_threshold.nc"）。
        start_year: int, 起始年份。
        end_year: int, 结束年份。

    功能：
        遍历输入的温度数据，为 03 至 10 月的每一天计算前后 15 天滑动窗口的 25%, 75% 和 90% 分位数，
        并将结果保存为 NetCDF 文件。
    """
    print("Loading data...")
    ...
    threshold_ds = xr.Dataset(
        {
            "tm_25th": (["day_of_year", "latitude", "longitude"], np.stack(all_25th, axis=0)),
            "tm_75th": (["day_of_year", "latitude", "longitude"], np.stack(all_75th, axis=0)),
            "tm_90th": (["day_of_year", "latitude", "longitude"], np.stack(all_90th, axis=0)),
        },
        coords={
...
if __name__ == "__main__":
    input_path = "/data6/huangty/ERA5/ERA5_downloads/tm_<year>_<month>.nc"

    calculate_heatwave_threshold(input_path=input_path)

    print("高温热浪阈值计算完成，并保存到指定文件。")
